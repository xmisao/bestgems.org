...
require 'database'
require 'batch/update_total_ranking'
require_relative '../run_migration'

class TestUpdateTotalRanking < MiniTest::Unit::TestCase
  def setup
    Gems.where.delete
    Value.where.delete
  ...
  def test_execute
    ...
    TotalRankingUpdater.execute(Date.new(2014, 6, 1))

    ranking = Ranking.where(:gem_id => 1).first
    assert_equal Ranking::Type::TOTAL_RANKING, ranking[:type]
